...
cdef void get_debiased_gradients_for_one_query(
    double* gains,
    double* preds,
    double* position_bias,  # same as t_plus
    long n_positions,
    long truncation_level,
    long* ranks0, 
    long start, 
    ...
    double max_log_arg,
    double log_idx_factor,
    int use_pairs_with_equal_gains
) nogil:
    cdef int tid = openmp.omp_get_thread_num()
    cdef int cnt
    cnt = <int>(end - start)
    ...
    cdef double p_labmda, p_hess
    cdef double gain_high, gain_low, score_high, score_low
    cdef double abs_delta_ndcg, paired_discount, multiplier
    cdef int high, low, rank_high, rank_low
    ...
    for i in range(cnt):
        ...
        for j in range(cnt):
            ...
            if (
                (gain_high > gain_low)
                or (
                    use_pairs_with_equal_gains != 0
                    and (high != low)
                    and (gain_high == gain_low)
                    and (gain_low != 0)
                    and (position_bias[rank_high] > position_bias[rank_low])
                )
            ):
                delta_score = preds[start + high] - preds[start + low]
                p_lambda = get_value(
                    delta_score,
                    sigmoid_table,
                    n_sigmoid_bins,
                ...
                if (gain_high > gain_low):
                    abs_delta_ndcg = (gain_high - gain_low) * paired_discount * inverse_max_dcg
                    p_lambda *= -abs_delta_ndcg / position_bias[rank_high]
                    p_hess *= abs_delta_ndcg / position_bias[rank_high]
                else:
                    abs_delta_ndcg = gain_high * paired_discount * inverse_max_dcg
                    multiplier = (
                        (position_bias[rank_high] - position_bias[rank_low])
                        / (position_bias[rank_low] * position_bias[rank_high])
                    )
                    p_lambda *= -abs_delta_ndcg * -multiplier
                    p_hess *= abs_delta_ndcg * multiplier
...
cdef void _get_debiased_gradients(
    double* gains, 
    double* preds,
    long* ranks0,
    long n_preds,
    ...
    for i in prange(n_queries, nogil=True):
        ...
        inverse_max_dcg = inverse_max_dcgs[i]
        get_debiased_gradients_for_one_query(
            gains, 
            preds,
            position_bias,
            n_positions,
...
def get_debiased_gradients(
  ...
  double[::1] grad,
  double[::1] hess,
  double[::1] position_bias,
  int use_pairs_with_equal_gains
):
    _get_debiased_gradients(
      (&gains[0]),
      (&preds[0]),
      (&ranks0[0]),
      n_preds,
